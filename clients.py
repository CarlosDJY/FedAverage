...
class client(object):
    ...
    def corrupt_client_data(self, target_label=7, corruption_ratio=0.1):
        ...
        """
        # 获取客户端的训练数据集
        data, labels = self.train_ds.tensors
        data = data.numpy()
        labels = labels.numpy()
        
        # 确定要更改的数据数量
        num_samples = len(labels)
        num_corrupt = int(num_samples * corruption_ratio)
        
        # 选择一部分数据添加后门并更改标签
        corrupt_indices = np.random.choice(num_samples, num_corrupt, replace=False)
        for idx in corrupt_indices:
            backdoored_image = data[idx].copy()
            # print(data[idx])
            backdoored_image[-6:] = 255
            data[idx] = backdoored_image
            labels[idx] = target_label
        
        # 更新客户端的训练数据集
        self.train_ds.tensors = (torch.tensor(data), torch.tensor(labels))
